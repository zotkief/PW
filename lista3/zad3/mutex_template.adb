...
-- Processes 
  ...
  Nr_Of_Processes : constant Integer :=2;
   ...
   ChosenProcess : Integer := 1 with Atomic;
   Requests : Choosing_Array := (others => False);
...
--    for Step in 0 .. Nr_of_Steps loop
    for Step in 0 .. Nr_of_Steps loop  -- TEST !!!
      -- LOCAL_SECTION - start
      ...
      Requests(Process.Id):=true;
      while Requests(OtherOne) loop
         if ChosenProcess=OtherOne then
            Requests(Process.Id):=false;
         end if;
         while ChosenProcess=OtherOne loop
            null;
         end loop;
         Requests(Process.Id):=true;
      end loop;
