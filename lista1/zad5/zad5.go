...
func traveler(symbol rune, Id int, seed int64, wg *sync.WaitGroup, ch chan []Trace, mu *[][]sync.Mutex) {
	...
	pos.x = Id
	pos.y = Id
	var direction int = int(math.Floor(gen.Float64()*2.0)) + (Id%2)*2
	tracer := []Trace{}
	...
	for i := 0; i < nr_of_steps; i++ {
		...
		var state bool = make_step(&pos, *gen, mu, &time_stamp, &tracer, symbol, Id, direction)
		if !state {
			symbol = symbol + 32
			tracer = append(tracer, *traceCreator(pos, time_stamp, symbol, Id))
			break
