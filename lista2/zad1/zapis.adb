...
procedure Travelers is
   ...
   protected type Ghost_Task_Type is
      procedure Init(Pos : Position_Type; Delay_Time : Duration);
      procedure Update_Position(Pos : Position_Type);
      procedure Finalize;
   ...
   protected body Ghost_Task_Type is
      ...
      procedure Init(Pos : Position_Type; Delay_Time : Duration) is
      begin
      ...
      procedure Update_Position(Pos : Position_Type) is
      begin
         Current := Pos;
         Time_Stamp := To_Duration(Clock - Start_Time);
         Store_Trace;

         if To_Duration(Clock - T0) > 3.0 * Max_Delay then
            Current.X := 15;
            Current.Y := 15;
            Time_Stamp := To_Duration(Clock - Start_Time);
            Store_Trace;
...
begin 
   for X in 0 .. Board_Width-1 loop
      for Y in 0 .. Board_Height-1 loop
         -- Tworzenie zadania dla komĂłrki na pozycji (X, Y)
